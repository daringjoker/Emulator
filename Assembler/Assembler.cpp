...
//            cout<<"added label "<<instr.labelname<<" for "<<currentAddress<<endl;
        }
        if(regex_match(instr.instruct,regex("^C(C|M|NC|NZ|PE|PO|P|Z)$|^J(NZ|MP|C|MP|NC|PO|PE|P|M|Z)$",std::regex_constants::icase)))
        {
            if(instr.arg1_type==tbd)
            instr.arg1_type=sTokenType::ref_label;
            else
                cout<<"kuch bura hua hai"<<endl;
