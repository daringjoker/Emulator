...
Instr_line Parser::parse_line(string line, int line_no) {
    ...
    } else if (pending_label && tokens[token_index].type == sTokenType::dec_label) reg_err(
            "Multiple label name for same line ")
    ...
    } else reg_err("Expected an instruction or a Label at the beginning of the line")
    ...
    } else reg_err("The first argument can only be an immediate,register or a label")
    if (tokens[token_index].type == sTokenType::comma) {
        ...
        if (tokens[token_index].type == sTokenType::reg || tokens[token_index].type == sTokenType::hex_imm ||
            ...
            if (tokens[token_index].type == sTokenType::lineend) {
                effInstr->num_token = token_index;
                return *effInstr;
            } else reg_err("An Instruction can have at most 2 Arguments in 8085 microprocessor")
        } else if (tokens[token_index].type == sTokenType::lineend) reg_err(
                "Second Argument is expected after the comma")
        else reg_err("The second argument can only be an immediate or a register")
    } else reg_err("A comma separator is expected between the arguments")
